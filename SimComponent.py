...
# ---------------- VT ABRAMS targeting ----------------
VT_CODES = {"VB"}                 # 2-char codes for VT Abrams (add more if you have them)
...
# ---------------- Rounding rules ----------------
LABOR_ROUND  = 3   # Base Rate (labor dollars)
BURDEN_ROUND = 5   # burden rates (CY#### columns)
COM_ROUND    = 6   # COM burden precision
...
def _derive_code_from_text(text) -> str | None:
    ...
    - Take first two visible chars, uppercase, strip non-alnum
    - If single digit, pad to 2 (4 -> '04')
    """
    s = str(text or "").strip().upper()
    if not s:
        return None
    two = (s[0] if len(s) else "") + (s[1] if len(s) > 1 else "")
...
def _derive_code_from_rbi_cell(val) -> str | None:
    """Normalize RBI 'Rate Band' to the same 2-char rule."""
...
def read_simplified_with_codes(xls: pd.ExcelFile) -> pd.DataFrame:
    ...
    # Find BUSINESS UNIT GDLS (or first non-year column)
    ...
    ren = {c: _normalize_cy(c) for c in year_cols_raw}
    df = df.rename(columns=ren)
    year_cols = [ren[c] for c in year_cols_raw]
...
def read_allowable_control_test(xls: pd.ExcelFile) -> pd.Series:
    """Return ACT series indexed by CY#### (rounded to 5)."""
    ...
    # Build descriptor from columns before first year column
...
def update_rate_band_import(simplified: pd.DataFrame, act: pd.Series, rbi_in: Path, rbi_out: Path):
    ...
    mask = pd.Series(False, index=rbi.index)
    if VT_CODES:
        mask |= rbi[rb_col].map(_derive_code_from_rbi_cell).isin(VT_CODES)
    if VT_DESC_TOKENS and desc_col:
        toks = {t.upper() for t in VT_DESC_TOKENS}
        m2 = rbi[desc_col].astype(str).str.upper().apply(lambda s: all(t in s for t in toks))
        mask |= m2
    if VT_REQUIRE_RATE_TYPE and rt_col:
        want = VT_REQUIRE_RATE_TYPE.upper()
        mask &= rbi[rt_col].astype(str).str.upper().eq(want)
...
def write_rateband_comparison(simplified_df: pd.DataFrame, rbi_path_or_df, out_path: Path):
    ...
    Build sets ONLY from:
      - RATSUM: first two chars of BUSINESS UNIT GDLS (we already computed 'rate_band')
      - RBI:    first two chars of the 'Rate Band' cell after normalization
    """
